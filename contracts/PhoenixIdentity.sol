...
contract PhoenixIdentity is Ownable {
    ...
    constructor(address _identityRegistryAddress, address _phoenixTokenAddress)
        public
    {
    ...
    function receiveApproval(
        ...
        require(
            _tokenAddress == phoenixTokenAddress,
        ...
        if (_bytes.length <= 32) {
            require(
                phoenixToken.transferFrom(sender, address(this), amount),
            ...
            require(
                phoenixToken.transferFrom(sender, via, amount),
                "Unable to transfer token ownership."
            );
    ...
    function handleAllowance(uint256 einFrom, uint256 amount) internal {
        ...
        resolverAllowances[einFrom][msg
            .sender] = resolverAllowances[einFrom][msg.sender].sub(amount);
    }
    ...
    function allowAndCall(
        ...
        if (amount != 0) {
            resolverAllowances[ein][destination] = resolverAllowances[ein][destination]
                .add(amount);
        }
